...
/*implicit declaration of transporting user message function*/
...
int main(int argc, char * argv[]) {
	...
	while ((opt = getopt_long(argc, argv, OPTION_R "k:a:C:thm", long_options, NULL)) != -1) {
        switch (opt) {
            case 'k':
				device_name = optarg;
	...
	while (1) {
		...
		if(message_mode){
			csp_print("Message to send: ");
			message = get_message(message);
			memcpy(packet->data, message, strlen(message));
			memset(packet->data + strlen(message), 0, 1);
			free(message);
		} else {
